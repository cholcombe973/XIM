...
public class CoreReceive extends Thread{
  ...
  //this needs to be loaded from the jar file
  ...
  //This is where it all happens with the sending, receiving and advertising.

  public CoreReceive(ReferenceManager rm){
    this.rm = rm;
  }
            ...
            if(checkForAdvertisements(sb.toString())){//If it's an advertisment drop the msg
              if(w!=null){                            //make sure w is a valid reference
                if(!soundPlaying){
                  ...
                  //zoom.play(imReceive);
                  zoom.playStream(((JarResourceLoader)rm.getReference("JarResourceLoader")).getResourceStream("receive.wav"));
                  zoom.start();
                }                                      //Play receive sound
                ...
                if(!f.isPublicWindowOpen()){
                  ...
                  if(!soundPlaying){
                    ...
                    JavaZoom zoom = new JavaZoom(this);
                    //zoom.play(imReceive);
                    zoom.playStream(((JarResourceLoader)rm.getReference("JarResourceLoader")).getResourceStream("send.wav"));
                    zoom.start();
                  }
